...
/*
may use more efficient algorithm to re-balance, the essence here is: the max # of shard each gid can have is avg+1
steps:
	1. construct a map gid -> [shard#]
	2. indirectly sort the map by len[shard#], get avg, others
	3. the max # of shards is avg+1, so use two pointer i = 0, j = len-1
	4. map[i] += avg-len(now), map[j] -= len(now)-avg+1 if others > 0;
	5. done
*/
		...
		for _, gid := range keys {
			...
			if count == avg {
				...
				for i, val := range config.Shards {
					if val == gid {
						if c == avg {
							config.Shards[i] = 0 //set to unused
						} else {
							c += 1
						}
					}
...
func (sm *ShardMaster) join(args JoinArgs) {
	...
	for k, v := range args.Servers {
		config.Groups[k] = v // how to handle old GID
	}

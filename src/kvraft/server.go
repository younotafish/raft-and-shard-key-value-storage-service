...
/*
in the msgNotify map, reqId is the address
*/
...
/*
get command from kv.applyCh -> do validation -> convert to op -> if not repeated, save changes to data -> safe
result to mail box(msgNotify), later msg can retrieve from it.
*/
...
func (kv *KVServer) waitApplyCh() {
	for {
		...
		case msg := <-kv.applyCh:
			...
			kv.saveSnapshot(msgIdx) // the logic is in the saveSnapshot method
...
/*
all the info are installed in the kv.data[]
*/

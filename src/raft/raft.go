...
const (
	ElectionTimeout  = time.Millisecond * 300 // usually the acual electionTimeOut = (1-2)* ElectionTimeout = (2-4)*HeartBeatTimeout
...
const (
	Follower  Role = 0 //default is follower
	Candidate Role = 1
	Leader    Role = 2
)
...
//
// restore previously persisted state. and initialize raft
//
...
/*
1. need to resetElectionTimer for candidate and leader. Follower has nothing to change
2. candidate should inc his term and vote for himself
3. leader should do some initialization including:
	1. nextIndex, which is initialized to its next index
	2. matchedIndex, which is used to get nextIndex for peers quickly.
*/
...
/*
get term and index for last entry of this raft
why not just the logEntry's index?
*/
...
//
/*
1. if the server is leader,append the command to its logEntries[], update its matchIndex and then persist
2. otherwise, nearly do nothing
*/
...
//
/*
why should we close channel?
1. kill the raft by setting the rf.dead to 1
2. By closing the channel, we can make all go routine done and return.
*/
func (rf *Raft) Kill() {
...
/*
1. every applyInterval raft will try to apply the logs, avoid overload the server
2. if the index of last applied < lastSnapshotIndex, we should installSnapShot first.
3. if commitIdx < lastApplied, either because new log don't arrive or don't install snapshot
4. regular case, append logs between lastApplied+1 and lastcommitIdx.
5. apply logs in this method is just sent it to raft.applyCh
*/
func (rf *Raft) startApplyLogs() {
	...
	if rf.lastApplied < rf.lastSnapshotIndex {
		...
		// have nothing to apply, either because no new log or didn't install snapshot
...
//这里有问题，应该   (2-3) heartbeats < r < (5-6) heartbeats(some reasonable duration, and it influence performance), and professor said the max is a few second
...
/*
1. applyCh is where raft should apply messages
2. what does gid do?
3. stopCh is used to lets go routine return quickly
4. timer is a powerful tool with combination of select clause, in this case:
	1. stopCh: used for quick return
	2.other timers: when time out, timers.C will receive a channel
5. the logic here is: when applytimer timeout or someone notify the raft, raft should try to apply logs
6. if election timeout, start election
7. eppendEntryTimeout is used to send heartbeat
